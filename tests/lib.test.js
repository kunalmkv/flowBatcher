...
describe("Ethers Library - approveTokens and executeBatchTransactions", () => {
  ...
  it("should approve tokens for batch transfer", async () => {
    ...
    expect(mockERC20Contract.approve.firstCall.args[1]).to.equal(
      ethers.parseUnits(amount.toString(), 18),
    )

    // Assert that the logger function was called correctly
    expect(
      logWithColorSpy.calledWith(
  ...
  it("should execute ERC-20 batch transfer correctly", async () => {
    ...
    expect(
      mockBatchTransferContract.batchTransfer.firstCall.args[0],
    ).to.deep.equal(recipients)
    expect(
      mockBatchTransferContract.batchTransfer.firstCall.args[1],
    ).to.deep.equal(amounts)
    expect(mockBatchTransferContract.batchTransfer.firstCall.args[2]).to.equal(
      tokenAddress,
    )
    ...
    expect(
      logWithColorSpy.calledWith(
        `Batch ERC-20 Transfer tx hash: 0xFakeBatchTxHash`,
        loggerColourEnum.INFO,
      ),
    ).to.be.true
    expect(
  ...
  it("should execute native ETH batch transfer correctly", async () => {
    ...
    expect(
      logWithColorSpy.calledWith(
        `Batch Native Transfer tx hash: 0xFakeBatchTxHash`,
        loggerColourEnum.INFO,
      ),
    ).to.be.true
    expect(
  ...
  it("should throw an error if the recipients and amounts arrays are not the same length", async () => {
    ...
    await expect(
      ethersLib.executeBatchTransactions(recipients, amounts, "0xTokenAddress"),
